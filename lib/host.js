...
const setup = async () => {
  ...
  await inquirer.prompt([
    ...
    },
    {
      type: "input",
      name: "clientPub",
      message: "Client Key (on client, 'icetty --key')",
      when: () => {
...
const start = () => {
  ...
  logger.info("signalling server: " + config.get("signallingServer"))

  const sub = waitForClients()
};

const waitForClients = () => {
  const hostChannel = config.get("channel");
  logger.info("host started " + hostChannel);

  return hub.subscribe(hostChannel)
