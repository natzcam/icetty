...
const start = async () => {
  ...
  logger.info(
    "connecting to signalling server: " + config.get("signallingServer")
  );
  hub = signalhub("icetty", [config.get("signallingServer")]);
  hub.subscribe(hostChannel).on("data", handleClient);
};
...
const startPty = (peer) => {
  ...
  return new Promise((resolve, reject) => {
    peer.on("data", (data) => {
      ...
      try {
        let pty = spawn(
          os.platform() === "win32" ? "powershell.exe" : os.userInfo().shell,
          [],
          {
            name: "icetty",
...
const isClientAllowed = (clientPub) => {
  ...
  return pubs.find((val) => val == clientPub);
};
