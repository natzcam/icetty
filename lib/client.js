...
const start = async (argv) => {
  ...
  await prompts.askConnectionDetails(argv, config);

  debug("starting client");

  try {
    hub = signalhub("icetty", [config.get("signallingServer")]);
...
const signalling = (hub, host) => {
  ...
  let timer;
  let signallingTimeout = process.env.DEBUG
    ? 1000000
...
const login = (peer) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    peer.on("data", async (loginResponse) => {
      debug("login response: %o", loginResponse.toString());
      if (loginResponse == "login" || loginResponse == "retry") {
        if (loginResponse == "retry") {
          console.log("Auth failed!");
        }

        const answers = await prompts.askLogin();
...
const session = (peer) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    process.stdout.on("resize", () => {
      peer.send(
        wrap(RESIZE, Buffer.from([process.stdout.columns, process.stdout.rows]))
      );
    });
